...
## [Build 26] — 2026-09-05
- **Bucko character on the site** — the mascot from the app icon now appears across getbucko.com, drawn from the same asset set the app ships (`assets/character/`, 13 palette-quantised PNGs, 184KB total). Hero: a waving Bucko leans in beside the phone mock (drops to a 150px figure above the phone on narrow screens). How it works: each of the three steps gets its illustration (adding a child, tapping the button, pay day), the same three beats the app's onboarding Welcome step uses. Features: the six stock Freepik icons are replaced by poses that match each card (tap, make the bed, streak calendar, family sync, kids mode lock, avatars). Pricing: a crowned Bucko peeks over the Most popular card. FAQ: a thinking Bucko under the heading. Footer: a waving Bucko on a paper disc beside the wordmark. Join page and 404 swap the square app icon for the family-sync and shrug poses.
- **Social preview card fixed** — `og:image` pointed every page at the square 1024px app icon while `twitter:card` declared `summary_large_image`, so link previews rendered a tiny cropped icon. New `assets/og-card.png` (1200×630: Bucko waving, the tagline with the marker highlight, the domain pill) is set on the home, privacy, terms, support and join pages with `og:image:width/height/alt`, `og:site_name`, and `twitter:title/description/image`; the JSON-LD `MobileApplication.image` points at it too.
- **Play feature graphic** — `assets/feature-graphic.png` (1024×500, 83KB) cropped from the streak-week scene for the Play Console listing (closes a TODO).
## [Build 25] — 2026-08-18
